...
router.post('/', async (req, res) => {
  ...
  const sendEmail = async (from, to) => {
    return axios.post(
      ...
      { from, to: [to], reply_to: email, subject: `New Enquiry from ${name} — Address Bali Villa`, html },
  ...
  try {
    ...
    const response = await sendEmail('Address Bali Villa <info@addressbaliuluwatu.com>', toEmail);
    ...
    try {
      const fallbackTo = process.env.FALLBACK_EMAIL || 'rigasnameji@gmail.com';
